...
Needs `DEEPSEEK_API_KEY` (env or root `.env`) since the summarization runs in the script, not the server, plus the workspace-root deps (`pnpm install`). Story lists come from [hckrnews.com](https://hckrnews.com/), whose archives (`/data/YYYYMMDD.js`) are keyed by UTC crawl day while the site displays local-timezone days — so the script merges the surrounding archive files, `latest.js`, and the server-rendered homepage, then cuts by local date, reproducing exactly the day sections the site shows. Any historical day works even after stories drop off the HN front page (`--list` prints a day's top stories without creating anything); comments come from the Algolia HN API. Article text is extracted with [Defuddle](https://github.com/kepano/defuddle) + linkedom (falls back to title + discussion when a site blocks fetching).
