...
- **`books.language` is user-set, not inferred.** It's an optional ISO-639-1 code driving which voices the picker offers first; `books.detectLanguage` (DeepSeek) only fills the same field on demand, so everything works with `DEEPSEEK_API_KEY` absent. The detector samples the **longest** chapter and prefers custom/clean over raw text — sampling the first chapter caught OCR noise on a title page and misread pre-reform Russian as Ukrainian. Don't set `maxTokens` on v4-flash; disable `thinking` instead, or the budget is spent before any answer is emitted.
