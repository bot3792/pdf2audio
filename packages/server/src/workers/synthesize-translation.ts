...
import { chapters, books, chapterTranslations } from "../schema.ts";
...
export async function synthesizeTranslation(
  ...
  const [current] = await db.select().from(chapterTranslations).where(eq(chapterTranslations.id, translationId));
  ...
  const transitioned = await db
    ...
    .where(and(eq(chapterTranslations.id, translationId), ne(chapterTranslations.audioStatus, "suspended")))
  ...
  try {
    ...
    if (current.status !== "done") {
      throw new Error(`Translation to ${current.language} is not finished`);
    }
    ...
    const slug = languageSlug(current.language);
    const wordCount = text.split(/\s+/).filter(Boolean).length;
    await chLog(`Synthesizing "${chapter.title}" in ${current.language} (${wordCount.toLocaleString()} words)`);
    ...
    const chunkPreviewDir = translationChunkPreviewDir(bookId, current.language, chapter.index);
    ...
    cancelPoll = setInterval(async () => {
      ...
      try {
        const [latest] = await db
          .select({ audioStatus: chapterTranslations.audioStatus })
    ...
    await ttsSynthesize({
      ...
      onProgress: async (chunk, totalChunks) => {
        const updated = await db.update(chapterTranslations)
    ...
    await chLog(`Done — ${Math.floor(totalSec / 60)}:${String(totalSec % 60).padStart(2, "0")} (${current.language})`);
    ...
    const rows = await db
      ...
      .where(and(
        ...
        eq(chapterTranslations.language, current.language),
    ...
    if (rows.length > 0 && remaining.length === 0) {
      if (suspended.length === 0) {
        await log(`All ${current.language} chapters synthesized, queuing assembly`);
        await addJob("assemble", { bookId, language: current.language }, { maxAttempts: 1 });
      } else {
        await log(`All queued ${current.language} chapters done (${suspended.length} suspended — queue them or assemble manually)`);
      }
    }
  } catch (err) {
    if (cancelPoll) {
    ...
    if (err instanceof TtsAbortedError) {
      await db.update(chapterTranslations)
