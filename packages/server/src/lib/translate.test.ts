...
import { splitForTranslation } from "./translate.ts";

describe("splitForTranslation", () => {
  it("keeps a short text as a single chunk", () => {
    expect(splitForTranslation("Hello there.\n\nSecond paragraph.")).toEqual([
      "Hello there.\n\nSecond paragraph.",
    ]);
  });
  ...
  it("groups paragraphs up to the size limit without splitting them", () => {
    ...
    const chunks = splitForTranslation([para, para, para, para].join("\n\n"));
  ...
  it("is deterministic", () => {
    ...
    expect(splitForTranslation(text)).toEqual(splitForTranslation(text));
  });

  it("drops empty paragraphs and whitespace-only text", () => {
    expect(splitForTranslation("  \n\n  \n\n")).toEqual([]);
    expect(splitForTranslation("A.\n\n   \n\nB.")).toEqual(["A.\n\nB."]);
  });
});
