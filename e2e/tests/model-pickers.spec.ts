...
test("registered models appear in pickers without a restart, grouped by source", async ({ page, fakeLlm }) => {
...
test("the chat picker shows no-tool models disabled instead of hiding them", async ({ page, fakeLlm }) => {
  await page.goto("/chat");
  const picker = page.getByTestId("chat-model");
